  ...
  class YouTubeStreamSync {
    ...
    constructor(options = {}) {
      this.options = {
        ...
        timezone: 0,
        quality: 'default',
        width: '100%',           // ← НОВЕ: ширина плеєра
        maxWidth: null,          // ← НОВЕ: максимальна ширина
        aspectRatio: '16:9',     // ← НОВЕ: співвідношення сторін
        customStyles: null,
        onReady: null,
        onPlay: null,
        onVideoChange: null,
    ...
    // Отримати весь плейлист
    getPlaylist() {
      return this.broadcastProgram;
    }

    // Отримати тільки список відео
    getSchedule() {
      if (!this.broadcastProgram) return [];
      return this.broadcastProgram.programSchedule;
    }

    // Отримати індекс поточного відео
    getCurrentIndex() {
      return this.currentVideoIndex;
    }

    // Перейти до конкретного відео
    goToVideo(index) {
    ...
    _calculateAspectRatio() {
      ...
      // Кастомне співвідношення "width:height"
      if (typeof ratio === 'string' && ratio.includes(':')) {
        const [w, h] = ratio.split(':').map(Number);
        return (h / w) * 100;
      }
      
      // Якщо передано число (наприклад 0.5625 для 16:9)
      if (typeof ratio === 'number') {
        return ratio * 100;
      }
      
      return 56.25; // За замовчуванням 16:9
    }
